...
export async function authUser(req: Request, res: Response, next: NextFunction) {
    ...
    try {
        ...
        if (blackListedToken) return res.status(401).json({ error: "Unauthorized access" });

        const decoded = jwt.verify(token as string, process.env.JWT_SECRET_KEY as string) as { email: string };
